...
\subsubsection{Dynamics following an ultrafast quench}

Let us examine the situation above in slightly more general terms.
% tdgl...

An interesting special case is when two order parameters compete with each other in the equilibrium free energy.
We consider a system with free energy density
\begin{equation}\label{eq:coupledfreeneergydensity}
f = f_1+f_2+f_c,
\end{equation}
where
\begin{equation}
f_i = -\frac{t_i}{2} \psi_i^2+\frac{u_i}{4}\psi_i^4+\frac{K}{2}(\nabla\psi_i)^2
\end{equation}
and
\begin{equation}
...
In the spirit of \cref{sec:nonthermalmelting}, we consider an ultrafast quench of the equilibrium order parameter $\psi_2$ which corresponds to a free energy density (\cref{eq:coupledfreeenergydensity}) with $t_1,t_2<0$.
Using \cref{eq:tdglequation}, \citet{sun_transient_2020} showed that, as the relaxation proceeds towards (meta)stability, there is a probability
\begin{equation}
p\sim 1-\zeta^\delta
\end{equation}
...
% Heating DONE
% Incoherent - melting of CDW DONE
% Incoherent can still be nonthermal - compare pumped sample to equilibrium phase diagram DONE

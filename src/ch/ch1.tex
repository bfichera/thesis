...
We start by writing down an effective free energy
\begin{equation}
F = -\left[\chi_{ijk}E^*_i(\omega_0)E_j(\omega_\mathrm{photon})E^*_k(\omega_\mathrm{photon}+\omega_0)+c.c.\right]
\end{equation}
with some tensor of coefficients $\chi_{ijk}$ (see \cref{ch:shgtheory}).
The effective field $\vec{P}$ produced by the pump pulse is given by
\begin{align}
P_i(\omega_0) &= -\frac{\partial F}{\partial E_i^*(\omega_0)}\\
&= \chi_{ijk}E_j(\omega_\mathrm{photon})E^*_k(\omega_\mathrm{photon}+\omega_0)\\
\end{align}
